        ...
        <div class=" bg-white overflow-hidden shadow-xl sm:rounded-lg p-4 ">
            ...
            <div class="flex-col space-y-4">
                <x-table class="overflow-x-auto">
                    <x-slot name='head'>
                        <x-table.heading sortable wire:click="sortBy('product_id')"
                            :direction="$sortField === 'product_id' ? $sortDirection :null" class="">{{ __('Product')}}
                        </x-table.heading>
                        ...
                        <x-table.heading sortable wire:click="sortBy('delegate')"
                            :direction="$sortField === 'delegate' ? $sortDirection :null">{{__("Delegate")}}
                        </x-table.heading>
                        <x-table.heading sortable wire:click="sortBy('delegate_phone')"
                            :direction="$sortField === 'delegate_phone' ? $sortDirection :null">{{__("Phone")}} 
                        </x-table.heading>
                        <x-table.heading>
                        </x-table.heading>
                    ...
                    <x-slot name='body'>
                        ...
                        <x-table.row class="cursor-pointer hover:bg-slate-200" wire:loading.class.delay="opacity-50 ">
                            <x-table.cell>{{$buy->product->name }}</x-table.cell>
                            <x-table.cell>{{$buy->quantity}}</x-table.cell>
                            <x-table.cell>{{$buy->delegate ?? ''}}</x-table.cell>
                            <x-table.cell>{{$buy->delegate_phone}}</x-table.cell>
                    
                            <x-table.cell>
                                <div class="space-x-1">


                                    <x-button wire:click="edit({{$buy->id}})">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 " fill="none"
                                            viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                        </svg></x-button>
                                    <x-danger-button class="ml-3" wire:click="confirmDelete({{$buy->id}})"
